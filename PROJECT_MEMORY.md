...
- 点检标准主表弹窗不再录入“所属车间”“所属区域”，标准适用位置由下方关联的设备/危化品对象体现，避免标准主表位置字段和适用范围重复。
...
- 所有基于场地底表的场地/车间/存放位置选择控件，需要按原一二级层级直接展示并支持选择：一级节点可选用于泛选，二级节点可选用于精确定位；不要把一级单独拎出重复展示，也不要在选项文字里展示场地编号。二级节点的显示值和保存值统一为“一级-二级”（如“铸造车间-二车间”），旧数据只存二级名称或场地编号时需要兼容识别并归一显示。选择一级时相关区域字段自动带入同名一级区域；点检标准/规则和危化品匹配逻辑也要同时识别“二级名”“场地编号”“一级-二级”，避免前端保存格式变化导致任务关联失效。
- 危化品台账列表在危化品较多时必须分页展示，默认每页 20 条；搜索、状态筛选、地点树切换后回到第一页，导出仍使用当前筛选结果全量导出。
- 点检报工异常项需要填写处理情况，默认为“待处理”；前端提交字段为 `handlingStatus`，用于后端生成隐患/整改工单，不写入点检明细底表不存在的字段。隐患表通过来源字段 `_widget_1777338377500` 区分业务来源：点检报工写“点巡检”，安全检查写“安全检查”。点检异常“待处理”沿用原 `flow_start` 发起待整改流程；“已处理”异常仍生成隐患留痕，但状态直接为“已关闭”并写关闭时间，不产生待整改任务。若同一次点检报工没有待处理异常，点检任务状态应闭环为“已完成”；只要存在待处理异常，任务仍为“异常”。
- 点检报工提交成功页需要与安全检查移动端提交后风格保持一致：只展示结果徽标、点检完成标题、设备/标准/总项/异常数/整改流程或已关闭工单等整体情况，以及返回任务首页/执行下一任务按钮；不要展示异常明细卡片，也不要在底部显示“报工已提交：提交ID”状态条。
...
- 安全检查记录页支持详情、编辑检查时间、打印/导出检查表，模板数据来自提交主表、提交明细和原检查模板；记录列表和详情中提交主表结束时间字段 `_widget_1776820784314` 统一展示为“检查时间”，编辑时只允许回写该字段。记录详情需要体现现场签字、异常说明、整改要求/措施、处理情况和异常照片，异常照片应以缩略图显示而不是仅显示链接。打印/导出中“检查人员”取主表签名子表 `_widget_1779262802768` 的手写签名字段 `_widget_1779262802794`，多个签名以 `、` 分隔，不取主表检查人/提交人；签名图片尺寸要受控，不能撑高签字栏，底部检查人员和检查日期两侧横线需要等高对齐。右侧“如不符合”列只填写异常项存在的问题和整改措施，正常项为空，不展示现场照片和签字时间。明细打印模板顶部保留检查日期和检查人员签字，不显示单位名称，底部不再重复检查人员和检查日期。检查日期取检查时间日期；该能力只放在安全检查记录页，不影响模板、任务和移动端提交页面。
- 安全检查记录页列表操作采用顶部菜单栏，打印/导出支持勾选多条记录批量处理；行内只保留详情操作，编辑检查时间入口放在详情弹窗右下角。点击编辑后应直接在当前详情弹窗内开放“检查时间”字段编辑权限，不再弹出二级编辑弹窗；保存仅用于修改检查时间。列表数据需要垂直居中，结果、异常数、状态、操作等短字段居中显示。记录页不显示右上角刷新按钮，也不显示底部“准备就绪”状态栏。
...
- 安全检查 H5 提交页现场信息区在“现场人员签名”子表下方提供“主要负责人签字（选填）”单独签名，不作为现场人员签名子表新增行；提交时上传后写入安全检查提交主表 `_widget_1780728739295`。执行检查底部提供“暂存”按钮，按任务/模板和区域对象保存本地草稿，草稿包含检查项结果、异常说明/整改要求/处理情况、异常照片、现场照片、现场人员签名和主要负责人签字；暂存成功后返回任务列表，下次打开同一任务提示恢复，提交成功后清除该草稿。
- 相关方入场培训填报主表支持是否危险作业 `_widget_1780365156595` 与危险作业附件上传 `_widget_1780365156616`；页面附件标题显示为“特种作业附件上传”。选择“是”时附件标题显示红色必填标记且必须上传附件，选择“否”时不强制。附件通过后端 `/api/partner-training/file-upload` 代理到培训记录底表 `c40d44a99118a61bd17f9325/upload_file` 后回填主表附件字段。
- 相关方入场培训管理页只承载培训记录查询、详情查看、批量打印和批量删除，不再左侧列表+右侧表格重复展示，也不内嵌二维码生成区；页头不显示刷新、新建、编辑按钮及说明副标题，标题下方工具栏左侧放二维码下载、打印、删除，右侧放搜索/重置，打印/删除按表格勾选记录批量执行但按钮文案始终为“打印”“删除”，不显示括号数量，未勾选记录时按钮也保持正常显示，点击后提示未选择数据；管理页不显示扫码提示蓝色 banner。记录详情通过弹窗查看，不放在数据列表下方；记录列表默认每页 10 条分页展示，支持切换 10/20/50 条；列表不显示编号、标题、施工项目，详情不显示施工项目。二维码下载单独使用 `partner-entry-training-qr.html` / `/h5/partner-training-qr` 页面。培训记录打印模板按《相关方入厂安全告知书》固定条款输出，只从数据带出相关方人员签名（参训人员子表签名图片，不展示姓名，未手写签名人员不展示）、施工单位名称和培训日期。
